...
export default function TechStartRebrandPage() {
  const { language, t } = useLanguage();
  const project = getProjectById('techstart-rebrand');
            ...
            >
              ...
              <div className="flex flex-wrap gap-4 sm:gap-6 mb-6 sm:mb-8">
                <div className="flex items-center gap-2 text-muted-foreground">
                  ...
                  <span className="text-xs sm:text-sm">{project.timeline[language]}</span>
                </div>
                {project.client && (
                  <div className="flex items-center gap-2 text-muted-foreground">
                    <Users size={16} className="sm:w-[18px] sm:h-[18px]" />
                    <span className="text-xs sm:text-sm">{project.client.name}</span>
                  </div>
                )}
              </div>
            </motion.div>
        ...
        <section className="px-4 sm:px-6 mb-12 sm:mb-16 md:mb-20">
          <div className="max-w-6xl mx-auto">
            <motion.div
              ...
              className={`aspect-video bg-gradient-to-br ${project.gradient} rounded-xl sm:rounded-2xl overflow-hidden relative`}
            >
              <div className="absolute inset-0 bg-card/30 flex items-center justify-center">
                <div className="text-4xl sm:text-5xl md:text-6xl opacity-40">📊</div>
              </div>
              <div className="absolute top-3 sm:top-4 md:top-6 left-3 sm:left-4 md:left-6">
                <span className="px-2 sm:px-3 md:px-4 py-1 sm:py-1.5 md:py-2 rounded-full bg-background/90 backdrop-blur-sm text-xs sm:text-sm font-medium">
                  {project.category[language]}
                </span>
              </div>
            </motion.div>
          ...
          <div className="max-w-6xl mx-auto">
            <div className="grid md:grid-cols-2 gap-4 sm:gap-6 md:gap-8">
              ...
              >
                  ...
                  <CardContent className="p-4 sm:p-6">
                    <div className="flex items-center gap-2 sm:gap-3 mb-3 sm:mb-4">
                      <Target className="text-destructive sm:w-6 sm:h-6" size={20} />
              ...
              >
                  ...
                  <CardContent className="p-4 sm:p-6">
                    <div className="flex items-center gap-2 sm:gap-3 mb-3 sm:mb-4">
                      <CheckCircle2 className="text-primary sm:w-6 sm:h-6" size={20} />
                      <h3 className="font-display text-xl sm:text-2xl font-bold">
                  ...
                  <Card className="bg-card/50 border-border/50 hover:border-primary/50 transition-all">
                    <CardContent className="p-4 sm:p-6">
                      <div className="flex gap-3 sm:gap-4">
                        <div className="flex-shrink-0 w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-primary/20 flex items-center justify-center text-primary font-bold text-base sm:text-lg">
                          {index + 1}
                        </div>
                ...
                >
                  <Card className="bg-card/50 border-border/50 hover:border-primary/30 transition-all h-full">
                    <CardContent className="p-4 sm:p-6 text-center">
                      ...
                      <p className="text-xs sm:text-sm font-semibold mb-1">{stat.label}</p>
                      {stat.description && (
                        <p className="text-[10px] sm:text-xs text-muted-foreground">{stat.description[language]}</p>
                      )}
                    </CardContent>
                  </Card>
                </motion.div>
